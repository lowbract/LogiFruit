...
function autocomplet(campo) {
	...
	if (keyword.length >= min_length) {
            ...
            if (radice === 'vett') { percorsophp = 'js/vettore.php?id='+idriga; }
            if (radice === 'auti') { percorsophp = 'js/autista.php?id='+idriga; }
            if (radice === 'astr') { percorsophp = 'js/trasporto.php?id='+idriga; }
		$.ajax({
			url: percorsophp,
			type: 'POST',
...
function setta_autista(campo,item,item2,tel1,tel2,numar) { //assegna nome e id
	// change input value
	$('#'+campo+'_txt').val(item);
        $('#'+campo+'_nom').val(item2);
        $('#'+campo+'_te1').val(tel1);
        $('#'+campo+'_te2').val(tel2);
        $('#'+campo+'_hid').val(numar);
	// hide proposition list
